...
//! Tools are saved to `~/.skyclaw/custom-tools/` with a companion metadata JSON file.
...
/// Adapter that wraps a script file as a SkyClaw Tool.
...
impl ScriptToolAdapter {
    ...
    pub fn from_meta_file(meta_path: &Path) -> Result<Self, SkyclawError> {
...
impl CustomToolRegistry {
    ...
    /// Load all custom tools from `~/.skyclaw/custom-tools/`.
    ...
    pub fn create_tool(
        ...
        let meta_json = serde_json::to_string_pretty(&meta)
            .map_err(|e| SkyclawError::Tool(format!("Cannot serialize tool metadata: {}", e)))?;
    ...
    pub fn delete_tool(&self, name: &str) -> Result<String, SkyclawError> {
        let meta_path = self.tools_dir.join(format!("{}.json", name));
        if !meta_path.exists() {
            return Err(SkyclawError::Tool(format!(
